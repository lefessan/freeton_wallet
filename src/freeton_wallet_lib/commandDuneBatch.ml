...
module GET = struct
  ...
  type 'a batch = {
    current_batch : 'a transferInfo array ;
    giver_addr : string ;
    batch_size : 'a ;
    ntransfers : 'a ;
    nprocessed : 'a ;
  ...
  type reply = string batch[@@deriving json_encoding]
end
...
let batch_of_reply ~reply =
  let batch = destruct "batch_of_reply" GET.reply_enc reply in
  (*
  let open GET in
